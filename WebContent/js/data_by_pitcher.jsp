...
try{
	...
	String sql = "select rank() over(order by round(sum(ER)*9/SUM(IP),3) asc) as rank, player_name, team_name, round(sum(ER)*9/SUM(IP),3) ERA, "
			+ "sum(TBF) TBF, sum(IP) IP, sum(H) H, sum(HR) HR, sum(BB) BB, sum(HBP) HBP, "
			+ "sum(SO) SO, sum(R) R, sum(ER) ER "
			+ "from pitcher_gamedata pg inner join player p on pg.player_id = p.player_id "
			+ "inner join team t on t.team_id = pg.team_id "
	...
	%>
	<table class = "result" id = "tablesort">
		<tr>
			<th>순위</th>
			<th>선수명</th>
			<th>팀</th>
